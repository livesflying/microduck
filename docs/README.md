...
The [README](../README_zh.md) is the front door — what a microduck is, and where to go. If you have
one in front of you and want to drive it, start at the [cheat sheet](robot/cheatsheet_zh.md).
...
| | |
|---|---|
| [`cheatsheet_zh.md`](robot/cheatsheet_zh.md) | Every `robotctl` command. |
| [`pair-a-gamepad_zh.md`](robot/pair-a-gamepad_zh.md) | Once per pad: pairing mode, `pad pair`, and what to do when it will not bond. |
| [`cheatsheet-dev_zh.md`](robot/cheatsheet-dev_zh.md) | The commands that need a dev board: branch builds, candidates, dev pushes. |
| [`dev-push_zh.md`](robot/dev-push_zh.md) | Build on your machine and install on the board over ssh, with no CI run. |
...
| | |
|---|---|
| [`architecture_zh.md`](design/architecture_zh.md) | The service split, the IPC contract, state ownership, safety and authority. |
| [`robotd-design_zh.md`](design/robotd-design_zh.md) | The control loop: the Dynamixel bus and who owns the port, the model, sensing, observations, policy, safety — and what else hangs off the tick. |
...
| [`policy-channel-design_zh.md`](design/policy-channel-design_zh.md) | Where the ONNX policies come from: the `policies` component, trying someone else's, and what `reset` puts back. |
| [`restart-order.md`](design/restart-order.md) | Which unit restarts, at which step, on every path that moves `current` — and at boot. |
| [`app-path-design_zh.md`](design/app-path-design_zh.md) | `btd` and `configd` — how a phone configures a robot over BLE. |
...
| [`remote-webrtc_zh.md`](design/remote-webrtc_zh.md) | WebRTC sessions, signalling, and the control channel — how a peer drives and observes the robot. |
...
| [`remote-access-design_zh.md`](design/remote-access-design_zh.md) | Reaching a duck from outside the LAN: the Hugging Face account, the device flow, and the bridge to a rendezvous service. |
...
| [`idle-cpu_zh.md`](project/idle-cpu_zh.md) | What the daemons do when nobody is asking them to: four things that stopped, two that were measured and left alone, and what still wants a board. |
